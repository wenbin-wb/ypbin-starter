 ...
 * 维度维护一个无 scope 的计数键——每次失败两个维度同步 +1，账号维先判定（先达阈值即先锁定）。
 * 因此<strong>任一达阈路径（含同一 IP 连续失败）都按账号维更长锁定时长（单维锁定分钟的固定倍数）
 * 生效</strong>，{@code (账号, scope)} 维的普通时长仅在账号维被重置后其残留计数仍达阈值的退化路径触发；
 * 该设计使轮换 IP 无法绕过账号锁定，代价是单 IP 高频误输的用户同样被升级为账号级长锁
 * （更严但安全方向正确，如业务需要区分请调整阈值/时长配置）。账号统一小写归一，避免大小写绕过。
 * 典型用法：</p>
 * <pre>{@code
