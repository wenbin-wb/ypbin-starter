...
- **网关：白名单路径携带令牌时仍识别身份**（`ypbin-starter-cloud-gateway`）。
  白名单路径（`ypbin.gateway.auth.exclude-paths`）过去命中后直接放行，**既不鉴权也不注入身份头**，
  导致「匿名可写、但同样需要用户维度」的端点拿不到身份——典型场景是埋点采集端点：匿名事件必须放行，
  但已登录用户的事件同样需要 `userId`/`tenantId`，否则落库后用户维度恒为空。
  - 现在的行为：**携带令牌**时尝试识别身份并写入可信身份头与来源标记；**无令牌**则原样放行
    （不调用鉴权器，匿名流量零额外开销）；令牌无效或鉴权器异常同样**按匿名放行**——只多识别一步，
    **绝不收紧鉴权**。
  - 安全前提（已逐条核实）：客户端伪造的身份头由 `HeaderSanitizeGlobalFilter` 清理，其顺序先于本过滤器；
    **未认证时不签发来源标记**，下游不会把客户端自带的身份头当作可信来源。
  - 新增 5 项用例（白名单+有效令牌 / 无令牌不触碰鉴权器 / 令牌被拒不签发标记 / 鉴权器异常按匿名放行 /
    非白名单未认证仍返回 401）。
