...
> 宿主（如 ypbin-admin）在自己机器上编译时就会解析到「从未发布」的代码。
> 项目生成器不受影响——它写入生成项目的是 CHANGELOG 里的最新已发布版本，而不是 `revision`。
> 若本地 `~/.m2` 已经装了「同名不同内容」的旧产物，删掉对应版本目录让它从中央仓库重新解析即可。
...
## 五、发布节奏约定（2026-08 起执行）

### 5.1 禁止"一修一发"（2026-09-16 新增，源自实际教训）

starter 的改动**一律先攒进 `CHANGELOG.md` 的 `[未发布]` 段**，不因为"刚修好一处"就发版。历史上多次出现
「修一处 → 顺手发一版 → 下一处修复又赶不上」，导致每次发版都留一截尾巴、版本号涨得比价值快。

### 5.2 只在明确触发点发版

满足其一才发：

1. **宿主被卡住**：某个已合并的委托方（admin / site / 其它项目）**因为缺少该修复而无法继续**，且绕不过去；
2. **成型里程碑**：`[未发布]` 攒成一个可命名的主题（例如"多客户端事件目录""可观测性收口"）。

不满足就继续攒 —— **"攒着"是默认动作，"发版"需要理由**。

### 5.3 发版前必须做一次"扫尾"

在动手改 `revision` 之前，逐条完成并在 PR/记录里留痕：

1. 逐条读 `[未发布]`，确认没有**半成品**（写了但没测、或只做了一半的同类改动）；
2. **主动找同类未修项**——教训：`@RefText` / `@DictText` / `LogDao` / `OnlineUserHelper` 同属
   "starter 定义端口、宿主必须实现"这一族，当只修了其中几个时，剩下的就是尾巴；
3. 核对**各宿主当前固定的 starter 版本**，列出它们缺哪些修复（`grep -n "ypbin-starter.version" */pom.xml`）；
4. 显式写一句"**现在发 or 继续攒**"及理由，而不是默认发。


避免"多版本同日发布、迭代节奏失真"，按真实开发节奏规划版本：
