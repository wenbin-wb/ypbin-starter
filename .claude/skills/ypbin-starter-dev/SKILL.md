...
面向 `ypbin-starter`（自研 Spring Boot starter 基础库合集，**JDK 21 / Spring Boot 4.1.x / Spring Cloud 2025.x**，发布到 Maven Central，groupId `cn.ypbin`）。它**不是业务应用，是被别人依赖的框架库**——每个能力模块提供「约定优于配置」的开箱即用能力，宿主项目（如 ypbin-admin）只 `<dependency>` 引入 + 少量 `ypbin.*` 配置即可用。定位与 ypbin-admin 的业务代码根本不同，规约也不同。

模块分层（共 35 个子模块，见 ROADMAP.md 与 site 文档）：
- **L1 基础层**（core / json / web / data / cache / log / security / i18n / tools / storage / api-doc / messaging / async / sign / api-crypto / social / sensitive-words / captcha / excel / job / license / ai 等）：不依赖 Spring Cloud，单体可用。
  - **security 模块内已含微服务身份头与平台访问能力**（不新增模块）：`identity` 包（`IdentityContext`/`IdentityHeaderFilter`/`IdentityHeaders`，微服务网关签发身份头 → 下游构建当前用户）与 `platform` 包（`@PlatformAccess`/`PlatformUserChecker`/`PlatformAccessAspect`，平台用户访问守卫）。自动装配注册于 `ypbin-starter-security` 的 `AutoConfiguration.imports`（`IdentityAutoConfiguration`/`PlatformAccessAutoConfiguration`，开关 `ypbin.security.identity.enabled` / `ypbin.security.platform.enabled`）。`UserContext` 已门面化：微服务身份头优先、Sa-Token 会话回退，业务代码无需感知部署形态。
- **L2 扩展层**（extension-crud / extension-tenant / extension-datapermission）：在 L1 上叠通用业务骨架。
- **L3 微服务层**（cloud-core / cloud-nacos / cloud-gateway / cloud-sentinel / cloud-loadbalancer / cloud-observability）：依赖 Spring Cloud。微服务宿主（如 ypbin-admin 微服务版）用 L1 的 identity/platform + L3 的 cloud-* 组合实现"网关统一鉴权 + 身份头透传"。cloud-core 内另含 `FeignResponses`（`cloud.feign.support`，v2.1.1 起）：统一解析 Feign 返回的 `R<T>`，`dataOrThrow(resp, msg)` 校验远程业务成功、失败抛 `BusinessException`（禁止静默降级），`dataOrDefault` 兜底可选值。
...
作者署名统一 `wenbin`。改动先用 Maven 编译/测试验证（见文末「验证」段，命令不绑定本机路径）。
...
11. **远程调用、空值防御与枚举规范**：
    - **远程调用防雪崩**：starter 内所有 HTTP / RPC / 客户端组件（如 Feign、RestTemplate、OkHttp、AI 客户端等）**必须显式提供并配置 `connectTimeout` 与 `readTimeout`**，严禁使用无超时的默认 Client，降级逻辑必须记录日志。
    - **集合返回值禁 null 与 IN 查询短路**：所有 public 方法返回 List/Set/Map 时查无数据一律返回空集合（`Collections.emptyList()`/`List.of()`），严禁返回 null；批量 `IN` 查询前**必须显式判空短路**（`if (isEmpty(ids)) return emptyList()`），严禁将空集合传入 `in(ids)` 导致 SQL 语法异常。
    - **枚举严禁 ordinal**：枚举必须显式定义 `private final int code` 与 `private final String desc`，严禁以 `Enum.ordinal()` 存库或传输。
    - **实体 equals 边界**：实体 `equals/hashCode` 严禁包含 List、关联对象或 `@TableField(exist=false)`，必须且仅基于主键 `id` 判等。
    - **禁止 `@Mapping` 纠正命名**：禁止使用 `@Mapping(source="a", target="b")` 掩盖命名分歧，Java 字段必须与 DB 列名严格同名对齐。
...
- 版本号统一走 root pom `<properties>` 的 `${revision}` + flatten-maven-plugin（**具体版本号见 root pom，不写死在此文档**）；模块 pom 不写死版本；新模块继承父 pom、依赖版本进 `ypbin-starter-dependencies`/`-bom` 管理，不在子模块散声明版本。
...
| R3 | 配置项非 `ypbin.*` 前缀；用散落 `@Value` 当模块配置入口；Properties 缺 `PREFIX` 常量或默认值 | 搜 `@ConfigurationProperties` / `@Value`，看 prefix 是否 `ypbin.`。**全量基线（2026-08 审核已全量收敛）：全部 `@ConfigurationProperties` 一律 `prefix = XxxProperties.PREFIX` + 类内 `public static final String PREFIX`，禁止硬编码字符串**（嵌套 `@Bean @ConfigurationProperties` 绑定 POJO 属例外，优先抽独立 Properties 类） |
...
> **本机 Git-bash 踩坑**：`grep -P`（PCRE，含中文字符类）在部分环境（Windows Git-bash）报 `-P supports only unibyte and UTF-8 locales`。查中文/复杂模式一律用 Grep 工具（内置 ripgrep），跨平台无此限制。
...
6. **JDK 基线为 21，别写更高版本 / preview 的 API**。本项目编译目标 JDK 21（`maven.compiler.release=21`），但开发者机器上可能还装着更新的 JDK 或 IDE 自带 JBR（可能 23+/25+），IDEA 会按语言级别放行部分高版本 API 导致「本地不飘红、命令行 release=21 编译不过」。已踩过的：`Locale.of(String, String)` 是 JDK 19+（旧基线 17 时缺失）；`Math.clamp`、`String` 的部分新方法、虚拟线程相关 API 等随版本推进要格外留意是否超出 21。写完务必用 JDK 21 命令行编译/测试一遍验证（见「验证」段），不能只靠 IDE 的语言级别提示。
...
8. **对外扩展点要有默认实现并明确「宿主可覆盖」语义**。starter 暴露的 SPI（如 `storage` 模块的 `FileRecorder` 文件元数据记录接口）必须自带默认实现（`StorageAutoConfiguration` 里的匿名空实现），并在接口 Javadoc 说明「业务方需要持久化元数据时实现并覆盖本 Bean」（`@ConditionalOnMissingBean` 保证可覆盖，铁律2）。**宿主侧检查**：启用对应能力模块时，确认是否需要自定义实现——admin 若未引入 starter-storage 则不受影响，一旦引入就必须决定 `FileRecorder` 落库方案，不能放任空实现静默丢元数据。

9. **AspectJ 复合切入点 `|| @within(注解)` 可能抛异常致切面静默失效**。`@Around("@within(A) || @within(B)")` 这类**复合**切入点，AspectJ 对部分注解（实测 Spring 的 `@Controller`）匹配时抛 `IllegalArgumentException: Type referred to is not an annotation type`，整个切入点匹配失败 → 目标类不被 AOP 代理 → 切面静默不生效（无报错、日志空，极难查）。`@RestController` 自带 `@Controller` 元注解，无需并列两个 `@within`；需覆盖多注解时先确认每个分支独立可匹配，或用 `execution` 类切入点。
...
本仓库是发布库，改动必须编译/测试验证。**协作开发各机器环境不同，以下命令不绑定任何具体路径**——用你本机可用的 JDK 21 + Maven 即可（IDEA 内置 Maven、sdkman、Homebrew、系统安装均可）：

1. **确认工具链**：JDK 必须是 21（与 `maven.compiler.release` 一致，别用更新的 JDK 造成 API 超前误判）。Maven 3.8+ 即可。若 `mvn`/`java` 不在 PATH，先配置好你本机的 JAVA_HOME / PATH，或用 IDE 内置 Maven：
   ```bash
   java -version    # 确认是 21
   mvn -v           # 确认可用
   ```
2. **编译/测试目标模块**（`test` 生命周期会自动执行已绑定的 `spotless:check`；改到被依赖的 L1 模块时加 `-am` 带上依赖模块一起编译）：
   ```bash
   mvn -pl <模块>[,<模块>...] -am test
   # 例如只验证某个模块：mvn -pl ypbin-starter-cache -am test
   ```
   - 若 spotless check 失败需要自动修复：先试 `mvn spotless:apply`（能解析插件前缀时）；不行则用完整坐标 `com.diffplug.spotless:spotless-maven-plugin:<当前版本>:apply`（版本见 `ypbin-starter-dependencies/pom.xml`），修复后重新执行 `test`。
3. **涉及装配逻辑的改动**，补/跑对应模块的 `ApplicationContextRunner` 测试，验证条件生效与可覆盖性；涉及 AOP 的用真实 Spring Boot 集成测试（PITFALL 9）。
4. **新增能力模块**记得同步：`.imports` 登记、父 pom `<module>` 声明、依赖版本进 dependencies/bom（PITFALL 5 / 配方段）。
5. **测试报找不到 JUnit/AssertJ**：检查该模块 pom 是否缺 `spring-boot-starter-test`（test scope）——它一并带来 JUnit5 + AssertJ + `MockHttpServletRequest`。
